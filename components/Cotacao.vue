<template>
  <div class="cotacao">
    <div>
      <p class="sobreTaxa">todas as moedas<br> com taxa<br> de transição nula</p>
    ...
    <div class="div">
      <div class="divCotacao" >
        ...
        <div class="valorMostrado">
          <p class="moeda">R$</p>{{ valorFinal.toFixed(2) }}
        </div>
        <input class="inputValor" type="number" placeholder="valor em BRL" v-model="valorTotal">
      </div>
      <div>
...
export default {
  ...
  mounted() {
    fetch(`https://economia.awesomeapi.com.br/last/USD-BRL,EUR-BRL,BTC-BRL,ETH-BRL,LTC-BRL,GBP-BRL`)
      .then((r) => r.json())
      .then((r) => {
...
.cotacao{
  ...
  align-items:center;
  height:90.5vh;
  background: linear-gradient(90deg, rgba(0,0,0,1) 50%, rgba(0,251,0,1) 50%,rgb(0,150,0));
}
.div{
  display:flex;
...
.cotacao{
  ...
  display:flex;
  flex-direction:column;
  background: linear-gradient(180deg, rgba(0,0,0,1) 50%, rgba(0,251,0,1) 50%,rgb(0,150,0));
}
.div{
  margin:.75rem;
}
